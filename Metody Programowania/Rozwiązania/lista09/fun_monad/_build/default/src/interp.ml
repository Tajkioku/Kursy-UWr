...
let rec eval_env (env : env) (e : expr) : value comp =
  ...
  (* | While (cond, body) ->
    fun heap0 ->
      let rec loop heap_acc =
        let (v_cond, heap1) = eval_env env cond heap_acc in
        match v_cond with
        | VBool true ->
            let (_, heap2) = eval_env env body heap1 in
            loop heap2
        | VBool false -> (VUnit, heap1)
        | _ -> failwith "while: oczekiwano bool"
      in
      loop heap0 *)
  (* Zad 2 *)
  | Break    -> fun _ -> raise BreakEx
  | Continue -> fun _ -> raise ContinueEx
  | While (cond, body) ->
    fun heap0 ->
      let rec loop heap_acc =
        let (v_cond, heap1) = eval_env env cond heap_acc in
        match v_cond with
        | VBool true ->
            (* podczas ewaluacji ciała łapiemy wyjątki *)
            let new_state =
              try
                let (_, heap2) = eval_env env body heap1 in heap2
              with ContinueEx -> heap1
                  | BreakEx    -> raise BreakEx  (* przekaż dalej *)
            in
            loop new_state
        | VBool false -> (VUnit, heap1)
        | _ -> failwith "while: oczekiwano bool"
